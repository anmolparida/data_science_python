drop table amazon_reviews_year_month_partitioned_clusterd;
drop table amazon_reviews_year_month_partitioned;
drop table amazon_reviews_year_month_partition_orc;
drop table amazon_reviews_table;
...
location '/user/hive/warehouse/Anmol_Parida/amazon_reviews_year_month_partitioned_ap';
...
location '/user/hive/warehouse/Anmol_Parida/amazon_reviews_year_month_partitioned_clustered_ap';
...
(reviewerid string, asin string, reviewername string, helpful array<int>, reviewtext string,
overall double, summary string, unixreviewtime bigint) partitioned by (yr int, mnth int)
stored as orc location '/user/hive/warehouse/Anmol_Parida/amazon_reviews_year_month_partition_orc_ap'
...
select helpful, helpful[0]/helpful[1] from amazon_reviews_table;
...
from amazon_reviews_table
where year(from_unixtime(unixreviewtime)) = 2008
and month(from_unixtime(unixreviewtime)) = 1; -- answer -0.029855284148784512


-- Graded Question 2

select  corr(size(split(reviewtext," ")), helpful[0]/helpful[1])
from amazon_reviews_table
where year(from_unixtime(unixreviewtime)) = 2008
and month(from_unixtime(unixreviewtime)) = 1; -- answer 0.1499
...
from amazon_reviews_table
where year(from_unixtime(unixreviewtime)) = 2008
and month(from_unixtime(unixreviewtime)) = 10; -- answer 206.25

-- Graded Question 4

select avg(overall)
from amazon_reviews_table
where size(split(reviewtext, ' ')) >= 206
and year(from_unixtime(unixreviewtime)) = 2008; -- Long Reviews - 4.071879854997411

select avg(overall)
from amazon_reviews_table
where size(split(reviewtext, ' ')) < 206;
and year(from_unixtime(unixreviewtime)) = 2008;     -- Short Reviews - 4.235906556253358

select avg(overall)
from amazon_reviews_table
where year(from_unixtime(unixreviewtime)) = 2008
and size(split(reviewtext, ' ')) < 206;             -- Short Reviews - 4.1612229952398385 (as per answer)


select overall, count(*) from amazon_reviews_table
group by overall;